...
def test_get_file_content(app2):
    file_id = 1
    dataset_id = 1

    with patch('app2.modules.dataset.services.DataSetService.get_or_404') as mock_get_or_404:
        mock_dataset = MagicMock()
        mock_dataset.id = dataset_id
        mock_dataset.name = "Dataset 1"
        mock_get_or_404.return_value = mock_dataset
        
        with patch('app2.modules.dataset.routes.render_template') as mock_render_template:
            with app2.app2_context():
...
def test_get_file_content_not_found(app2):
    file_id = 1
    dataset_id = 999 
...
def test_delete_file(app2):
    file_name = "test_file.txt"

    file_path = os.path.join('/mock/temp/folder', file_name)

    with patch('app2.modules.dataset.routes.current_user') as mock_user:
...
def test_delete_file_negative(app2):
    file_name = "test_file.txt"
